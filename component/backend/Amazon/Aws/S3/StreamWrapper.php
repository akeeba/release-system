...
use Akeeba\ARS\Amazon\Aws\S3\Exception\NoSuchKeyException;
use Akeeba\ARS\Amazon\Aws\S3\Iterator\ListObjectsIterator;
use Akeeba\ARS\Amazon\Guzzle\Http\EntityBody;
use Akeeba\ARS\Amazon\Guzzle\Http\CachingEntityBody;
use Akeeba\ARS\Amazon\Guzzle\Http\Mimetypes;
use Akeeba\ARS\Amazon\Guzzle\Iterator\FilterIterator;
use Akeeba\ARS\Amazon\Guzzle\Stream\PhpStreamRequestFactory;
...
class StreamWrapper
{
    ...
    protected function openReadStream(array $params, array &$errors)
    {
        ...
        $this->body = $factory->fromRequest($request, array(), array('stream_class' => 'Akeeba\\ARS\\Amazon\\Guzzle\\Http\\EntityBody'));
