...
namespace Akeeba\ARS\Amazon\Aws\Common\Client;
...
use Akeeba\ARS\Amazon\Aws\Common\Credentials\NullCredentials;
use Akeeba\ARS\Amazon\Aws\Common\Enum\ClientOptions as Options;
use Akeeba\ARS\Amazon\Aws\Common\Exception\ExceptionListener;
use Akeeba\ARS\Amazon\Aws\Common\Exception\InvalidArgumentException;
use Akeeba\ARS\Amazon\Aws\Common\Exception\NamespaceExceptionFactory;
use Akeeba\ARS\Amazon\Aws\Common\Exception\Parser\DefaultXmlExceptionParser;
use Akeeba\ARS\Amazon\Aws\Common\Exception\Parser\ExceptionParserInterface;
use Akeeba\ARS\Amazon\Aws\Common\Iterator\AwsResourceIteratorFactory;
use Akeeba\ARS\Amazon\Aws\Common\RulesEndpointProvider;
use Akeeba\ARS\Amazon\Aws\Common\Signature\EndpointSignatureInterface;
use Akeeba\ARS\Amazon\Aws\Common\Signature\SignatureInterface;
use Akeeba\ARS\Amazon\Aws\Common\Signature\SignatureV2;
use Akeeba\ARS\Amazon\Aws\Common\Signature\SignatureV3Https;
use Akeeba\ARS\Amazon\Aws\Common\Signature\SignatureV4;
use Akeeba\ARS\Amazon\Guzzle\Common\Collection;
use Akeeba\ARS\Amazon\Guzzle\Plugin\Backoff\BackoffPlugin;
use Akeeba\ARS\Amazon\Guzzle\Plugin\Backoff\CurlBackoffStrategy;
use Akeeba\ARS\Amazon\Guzzle\Plugin\Backoff\ExponentialBackoffStrategy;
use Akeeba\ARS\Amazon\Guzzle\Plugin\Backoff\HttpBackoffStrategy;
use Akeeba\ARS\Amazon\Guzzle\Plugin\Backoff\TruncatedBackoffStrategy;
use Akeeba\ARS\Amazon\Guzzle\Service\Description\ServiceDescription;
use Akeeba\ARS\Amazon\Guzzle\Service\Resource\ResourceIteratorClassFactory;
use Akeeba\ARS\Amazon\Guzzle\Log\LogAdapterInterface;
use Akeeba\ARS\Amazon\Guzzle\Log\ClosureLogAdapter;
use Akeeba\ARS\Amazon\Guzzle\Plugin\Backoff\BackoffLogger;
            ...
            if ($logger === 'debug') {
                ...
                throw new InvalidArgumentException(
                    Options::BACKOFF_LOGGER . ' must be set to `debug` or an instance of '
                        . 'Akeeba\\ARS\\Amazon\\Guzzle\\Common\\Log\\LogAdapterInterface'
                );
            }
    ...
    protected function getSignature(ServiceDescription $description, Collection $config)
    {
        ...
        if (is_string($signature)) {
            ...
            throw new InvalidArgumentException('The provided signature is not '
                ...
                . 'Akeeba\\ARS\\Amazon\\Aws\\Common\\Signature\\SignatureInterface');
        }
