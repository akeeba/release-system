...
namespace Akeeba\Component\ARS\Administrator\View\Autodescriptions;
...
class HtmlView extends BaseHtmlView
{
	...
	{
		/** @var AutodescriptionsModel $model */
		$model               = $this->getModel();
	...
	private function addToolbar()
	{
		...
		ToolbarHelper::title(sprintf(Text::_('COM_ARS_TITLE_AUTODESCRIPTIONS')), 'icon-ars');
		...
		if ($canCreate)
		{
			ToolbarHelper::addNew('autodescription.add');
		}
		...
		if ($canDelete || $canEditState || $canCreate)
		{
			...
			if ($canEditState)
			{
				$childBar->publish('autodescriptions.publish')
					->icon('fa fa-check-circle')
					->text('JTOOLBAR_PUBLISH')
					->listCheck(true);

				$childBar->unpublish('autodescriptions.unpublish')
					->icon('fa fa-times-circle')
					->text('JTOOLBAR_UNPUBLISH')
					->listCheck(true);

				$childBar->checkin('autodescriptions.checkin')->listCheck(true);
			}

			if ($canDelete)
			{
				$childBar->delete('autodescriptions.delete')
