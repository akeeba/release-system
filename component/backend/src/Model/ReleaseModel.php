...
class ReleaseModel extends AdminModel
{
	...
	/**
	 * Authorises the CURRENT category of every batched release before Joomla's batchMove()/batchCopy()
	 * overwrite `category_id` (and, for copy, zero out the primary key) on the table object passed to
	 * onBeforeBatch(). By the time that event fires there is no way to recover which category a release
	 * is actually being taken FROM, so this must run first, against the unmutated database rows — otherwise
	 * a user with rights on the target category alone could move or duplicate a release out of a source
	 * category they have no access to.
	 *
	...
	private function assertSourceCategoryAccessForBatch(array $commands, array $pks): void
	{
		...
		$query = DbQuery::create($db)
			->select([$db->quoteName('id'), $db->quoteName('category_id')])
			->from($db->quoteName('#__ars_releases'))
